...
def create(name, type, description, endpoint, config, capabilities, tags, save, output):
    ...
    try:
        ...
        tool_table.add_row("Capabilities", ', '.join([cap.name if hasattr(cap, 'name') else str(cap) for cap in tool.capabilities]))
        ...
        if save:
            try:
                ...
                tool_data = {
                    ...
                    "capabilities": [cap.name if hasattr(cap, 'name') else str(cap) for cap in tool.capabilities],
                    "tags": tool.tags,
                    "config": config_dict,
...
@click.option("--method", type=click.Choice(['GET', 'POST', 'PUT', 'DELETE']), default='GET', help="HTTP method for API tools")
...
def test(name, input, method, params):
    ...
    try:
        ...
        if tool_type == 'api':
            ...
            mock_output = f"Tool '{name}' executed successfully with input: {input_data}"
