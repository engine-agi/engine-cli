...
def mock_protocol_builder():
    with patch('engine_cli.commands.protocol.ProtocolBuilder', return_value=MockProtocolBuilder()) as mock_builder:
        yield mock_builder
...
class TestProtocolStorage:
    ...
    def test_list_protocols(self, mock_protocol_storage):
        ...
        with patch('engine_cli.commands.protocol.protocol_storage', mock_protocol_storage):
...
class TestProtocolCLICommands:
    ...
    def test_create_protocol_basic(self, cli_runner, mock_protocol_enums, mock_protocol_builder, mock_imports):
        """Test creating a basic protocol"""
        with patch('engine_cli.commands.protocol.success'), \
             patch('engine_cli.commands.protocol.table'), \
             patch('engine_cli.commands.protocol.print_table'), \
    ...
    def test_create_protocol_full_options(self, cli_runner, mock_protocol_enums, mock_protocol_builder, mock_imports):
        ...
        with patch('engine_cli.commands.protocol.success'), \
            ...
            result = cli_runner.invoke(create, [
                'full_protocol',
                '--description', 'Full featured protocol',
                '--author', 'Test Author',
                '--version', '2.0.0',
                '--tags', 'test,development,production',
                '--intents', 'analysis,development',
                '--command-types', 'task_execution',
                '--scope', 'project',
                '--strict-validation',
                '--save'
            ])
    ...
    def test_create_protocol_import_error(self, cli_runner):
        ...
        with patch('engine_cli.commands.protocol.ProtocolBuilder', side_effect=ImportError("No module")):
            from engine_cli.commands.protocol import create
    ...
    def test_list_protocols_with_filters(self, cli_runner, mock_protocol_storage):
        ...
        with patch('engine_cli.commands.protocol.protocol_storage', mock_protocol_storage), \
            ...
            result = cli_runner.invoke(list, ['--tag', 'test', '--author', 'Test Author'])
    ...
    def test_test_protocol_success(self, cli_runner, mock_protocol_storage, mock_protocol_enums):
    ...
    def test_test_protocol_no_command(self, cli_runner, mock_protocol_storage):
        ...
        with patch('engine_cli.commands.protocol.protocol_storage', mock_protocol_storage):
            ...
            assert result.exit_code == 1 or "Please provide a command to test" in result.output
