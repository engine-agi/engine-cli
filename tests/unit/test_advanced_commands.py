...
from engine_cli.commands.advanced import monitor, health, logs
...
class TestAdvancedCommands:
    ...
    def test_monitor_command_json(self, runner):
        ...
        result = runner.invoke(monitor, ['--json'])
        assert result.exit_code == 0
    ...
    def test_health_command_detailed(self, runner):
        ...
        result = runner.invoke(health, ['--detailed'])
        assert result.exit_code == 0
...
class TestBulkOperations:
    ...
    def test_bulk_create_agents(self, runner):
        ...
        result = runner.invoke(runner.invoke, ['advanced', 'bulk', 'create-agents', 'agent1', 'agent2'])
    ...
    def test_bulk_agents_operation(self, runner):
        ...
        result = runner.invoke(runner.invoke, ['advanced', 'bulk', 'agents', 'test*', '--action', 'start', '--dry-run'])
        assert result is not None
...
class TestConfigOperations:
    ...
    def test_config_export(self, runner, tmp_path):
        ...
        with patch('engine_cli.commands.advanced.load_config') as mock_load:
            ...
            result = runner.invoke(runner.invoke, ['advanced', 'config-ops', 'export', str(output_file)])
            assert result is not None

    def test_config_import_dry_run(self, runner, temp_config_file):
        """Test config import command with dry run."""
        result = runner.invoke(runner.invoke, ['advanced', 'config-ops', 'import-config', str(temp_config_file), '--dry-run'])
        assert result is not None

    def test_config_import_merge(self, runner, temp_config_file):
        """Test config import command with merge."""
        with patch('engine_cli.commands.advanced.load_config') as mock_load:
            with patch('engine_cli.commands.advanced.EngineConfig') as mock_config_class:
                with patch('engine_cli.commands.advanced.save_config') as mock_save:
                    ...
                    result = runner.invoke(runner.invoke, ['advanced', 'config-ops', 'import-config', str(temp_config_file), '--merge'])
                    assert result is not None