...
class TestWorkflowFunctions:
    ...
    def test_test_workflow_success(self, mock_engine_class, mock_storage, mock_resolver_class):
    ...
    @patch('engine_cli.commands.book_backup.workflow_storage')
    @patch('engine_cli.commands.book_backup.WorkflowEngine')
    def test_test_workflow_with_input_data(self, mock_engine_class, mock_storage, mock_resolver_class):
        ...
        # Mock engine
        mock_engine = MagicMock()
        mock_engine.run.return_value = {'status': 'completed', 'result': 'validation_passed'}
        mock_engine_class.return_value = mock_engine

        input_data = '{"test_input": "sample_data", "expected_output": "result"}'
        result = self.runner.invoke(cli, ['test', 'test_data_workflow', '--input-data', input_data])
        assert result.exit_code == 0

    @patch('engine_cli.commands.book_backup.WorkflowResolver')
    @patch('engine_cli.commands.book_backup.workflow_storage')
    @patch('engine_cli.commands.book_backup.WorkflowEngine')
    def test_test_workflow_invalid_json_input(self, mock_engine_class, mock_storage, mock_resolver_class):
    ...
    @patch('engine_cli.commands.book_backup.workflow_storage')
    @patch('engine_cli.commands.book_backup.WorkflowEngine')
    def test_test_workflow_execution_failed(self, mock_engine_class, mock_storage, mock_resolver_class):
    ...
    @patch('engine_cli.commands.book_backup.workflow_storage')
    @patch('engine_cli.commands.book_backup.WorkflowEngine')
    def test_test_workflow_resolution_failed(self, mock_engine_class, mock_storage, mock_resolver_class):
