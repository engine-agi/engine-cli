...
class TestMainCLI:
    ...
    def test_status_command_all_available(self, mock_workflow, mock_team, mock_agent, runner):
    ...
    def test_status_command_core_unavailable(self, mock_workflow, mock_team, mock_agent, runner):
    ...
    def test_team_group_exists(self, runner):
        ...
        assert "Manage agent teams" in result.output or "Team management commands" in result.output
    ...
    def test_agent_group_import_error(self, mock_import, runner):
        ...
        result = runner.invoke(cli, ['agent', '--help'])
        # When import fails, the group should still exist but show error on commands
        assert result.exit_code == 0
    ...
    @patch('engine_cli.commands.workflow.cli', side_effect=ImportError("Module not found"))
    def test_workflow_group_import_error(self, mock_import, runner):
        ...
        assert "Workflow commands not available" in result.output or result.exit_code == 0
    ...
    @patch('engine_cli.commands.protocol.cli', side_effect=ImportError("Module not found"))
    def test_protocol_group_import_error(self, mock_import, runner):
        ...
        assert "Protocol commands not available" in result.output or result.exit_code == 0
    ...
    @patch('engine_cli.commands.project.cli', side_effect=ImportError("Module not found"))
    def test_project_group_import_error(self, mock_import, runner):
        ...
        assert "Project commands not available" in result.output or result.exit_code == 0

    @patch('engine_cli.commands.examples.cli', side_effect=ImportError("Module not found"))
    def test_examples_group_import_error(self, mock_import, runner):
        ...
        assert "Examples commands not available" in result.output or result.exit_code == 0

    @patch('engine_cli.commands.config.cli', side_effect=ImportError("Module not found"))
    ...
    @patch('engine_cli.commands.advanced.cli', side_effect=ImportError("Module not found"))
    def test_advanced_group_import_error(self, mock_import, runner):
        ...
        assert "Advanced commands not available" in result.output or result.exit_code == 0

    @patch('engine_cli.commands.monitoring.cli', side_effect=ImportError("Module not found"))
    def test_monitoring_group_import_error(self, mock_import, runner):
        """Test monitoring group when import fails."""
        result = runner.invoke(cli, ['monitoring', '--help'])
        assert result.exit_code == 0
        result = runner.invoke(cli, ['monitoring'])
        assert "Monitoring commands not available" in result.output or result.exit_code == 0